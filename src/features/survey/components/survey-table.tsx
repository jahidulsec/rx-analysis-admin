...
export default function SurveyTable({
  ...
  const columns: ColumnDef<Survey>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span>
          {row.original.territoryName} ({row.original.territoryId})
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span>
          {row.original.surveyorName} ({row.original.createdBy})
        </span>
      ),
    },
    {
        ...
        onAction={() => {
          startTransition(async () => {
            if (del && typeof del !== "boolean") {
              const response = deleteSurvey(del);
